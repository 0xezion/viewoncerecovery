...
def executar_curl_download_verbose(url, enc_path, id_row, local_logs):
    """
    Executa a resolução de DNS via socket nativo e efetua o download via cURL.
    Retorna uma tupla (sucesso, ip_resolvido).
    """
    startupinfo = None
    if os.name == 'nt':
    ...
    # Resolução Ética e Nativa de DNS via API do Sistema Operacional
    try:
        dominio = urlparse(url).netloc
        if ":" in dominio:  # Remove a porta caso exista na string do domínio
            dominio = dominio.split(":")[0]
            
        ip_resolvido = socket.gethostbyname(dominio)
        local_logs.append((f"[DNS-FORENSE] Domínio '{dominio}' resolvido para: {ip_resolvido}", "green"))
    except Exception as dns_err:
        ip_resolvido = "Falha na resolução de DNS"
        local_logs.append((f"[DNS-ALERTA] Não foi possível mapear o domínio pelo socket: {dns_err}", "yellow"))
    ...
    try:
        ...
        sucesso = os.path.exists(enc_path) and os.path.getsize(enc_path) > 0
        return sucesso, ip_resolvido
...
def processar_registro_individual(row, output_dir, enc_folder, ui_queue):
    ...
    def build_result(status_crypto, path="N/A", size=0, url_str="N/A"):
        return {
            ...
            "dns_ip": ip_baixado, "sha256": sha256_hash
        }
    ...
    # Execução do download coletando o IP via socket de forma desacoplada do cURL
    download_success, ip_baixado = executar_curl_download_verbose(url, enc_path, id_row, local_logs)
    ...
    try:
        ...
        # Carga e geração do Hash SHA256 da mensagem recuperada
        sha256_hash = hashlib.sha256(decrypted_bytes).hexdigest()
        ...
        if ext_real:
            ...
            local_logs.append((f" -> SHA256 da Mensagem: {sha256_hash}", "green"))
...
def generate_report(entries, output_dir, db_version, total_tables):
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        # Estilo para destacar os novos dados inseridos (IP e SHA256)
        f.write(".novo-dado{background:#f0f4f8 !important; font-weight:bold; color:#0b3c5d !important;}")
        ...
        f.write(f"<div class='subtitle-bar'><b>Geração do Artefato:</b> {datetime.now().strftime('%d/%m/%Y %H:%M:%S')} UTC | <b>Amostras Evaluadas:</b> {len(entries)}</div>")
        ...
        for e in ok_rec:
            ...
            # --- POSICIONAMENTO EXATO DOS CAMPOS CONFORME SOLICITADO ---
            f.write(f"<tr><td class='title'>URL Completa Origem</td><td><code>{e['url_bruta']}</code></td></tr>")
            # 1. IP baixado logo abaixo da URL
            f.write(f"<tr><td class='title novo-dado'>Endereço IP (DNS)</td><td class='novo-dado'><code>{e['dns_ip']}</code></td></tr>")
            
            f.write(f"<tr><td class='title'>Chave Simétrica (Hex)</td><td><code>{e['key_material']}</code></td></tr>")
            # 2. SHA256 logo abaixo do campo Hex (Chave Simétrica / Material Hex)
            f.write(f"<tr><td class='title novo-dado'>SHA256 (Recuperado)</td><td class='novo-dado'><code>{e['sha256']}</code></td></tr>")
        ...
        for e in fail_rec:
            ...
            f.write(f"<tr><td class='title'>URL Completa Origem</td><td><code>{e['url_bruta']}</code></td></tr>")
            # IP do DNS também mapeado aqui para fins de rastreabilidade de requisição bloqueada/expirada
            f.write(f"<tr><td class='title novo-dado'>Endereço IP (DNS)</td><td class='novo-dado'><code>{e['dns_ip']}</code></td></tr>")
...
class AppForense(tk.Tk):
    ...
    def build_ui(self):
        ...
        self.txt_log = tk.Text(frame_log, bg="#0a0f1d", fg="#00e5ff", font=("Consolas", 9), wrap="none")
